...
class ClassBoardApp(QWidget):
    ...
    def _show_menu(self, pos):
        ...
        m.addSeparator()
        m.addAction("检查更新", self.open_updater)
    ...
    def _auto_check_update(self):
        if not self.settings.get("check_update_on_start", True):
            return
        # silent=True: 无新版静默；auto=False: 有新版弹窗让用户决定
        self._open_updater(auto=False, silent=True)

    def open_updater(self):
        """右键菜单 / 设置里手动触发。"""
        self._open_updater(auto=False, silent=False)

    def _open_updater(self, auto, silent):
        try:
            ...
            return
        ok, msg = launch(auto=auto, silent=silent)
